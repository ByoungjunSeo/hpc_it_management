...
- **BUG-15** — 반납 시 컴퓨팅 모듈 재고 미차감·노드 사용중 잔존 → 섀시 연쇄 반납+recalc [완료]
- **BUG-15-b** — 구 코드 반납분 재고/이력 소급 보정(게이트 SQL, 운영 실행은 승인 후) [설계 완료·게이트]
- **BUG-15-c** — 반납 시 모듈 removed 이력 자동 기록(트랜잭션 내, 이중 차감 없음) [완료]
- **BUG-16** — 사용 현황 중복 행 → GROUP BY 합산 + 이동 시 동일 슬롯 병합 [완료]
- **BUG-17** — 미매칭 화면 'AI 스펙 조회'(Ollama) 제거, 외부 IP 노출 경로 소멸 [완료]

> v2.2.0(태그·tar) 자체에는 BUG-1~13 반영. BUG-14~17은 태그 이후 수리 → **v2.2.1로 합류**(tar-태그 일관성).
