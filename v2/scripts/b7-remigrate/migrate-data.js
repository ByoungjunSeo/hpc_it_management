...
// [B-7c'] 이관 소스는 B-7a 보존 스냅샷 고정. 운영 v1 DB 직접 접근 금지.
const SQLITE_PATH = process.env.B7_SQLITE_PATH || '/tmp/v1_snapshot_b7a.sqlite';
...
async function main() {
  ...
  // [B-7c'] host/port는 env에서 취득(원본은 5433 하드코딩이었음) + 실행 직전 가드
  const pgPort = parseInt(process.env.PGPORT || '5434', 10);
  assertScratchTarget(pgPort);
  console.log(`[대상] SQLite=${SQLITE_PATH} → PostgreSQL=${process.env.PGHOST || '127.0.0.1'}:${pgPort}`);
  const pg = new Pool({
