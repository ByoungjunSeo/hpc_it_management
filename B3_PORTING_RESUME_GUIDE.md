...
> 작성일: 2026-06-26 / 최종 현행화: 2026-07-15 (BUG-9/10 수리)
> 작성 목적: 서버 종료 전 맥락 보존. 재개 시 이 문서 + git log부터 확인.

> ★ **BUG-9/10 수리 (2026-07-15, 운영 반영 대기)**: BL-2 후속 조사에서 드러난 2건.
> **BUG-9** — 자식 노드 사용등록이 부모 섀시 위치를 덮어씀(inventory.js 동기화 블록 원인) →
> 블록 제거 + 모델 가드(자식은 위치 필드 강제 NULL) + 폼 위치 숨김. **BUG-10** — 다노드 입고
> 노드의 blade_slot 미저장 + blade_slot 의미 충돌(렌더용 left/right vs 노드슬롯) → **node_index
> 신설**(블레이드 번호 전용), blade_slot 렌더용 원복, 부분 유니크 인덱스 (parent, node_index)로 교체,
> 입고·BL-2 양 경로 node_index 부여. 마이그레이션 `db/migrations/2026-07-15_1_bug9_10_node_index.sql`
> + 데이터 보정 게이트 SQL. 상세: BUG_TRACKING.md BUG-9/10.
>
> ★ **검증 관례 추가(이 마디부터)**: **격리 검증은 HTTP 실경로 필수**(로그인 세션 curl/fetch로
> 라우트를 실제로 탄다). 모델 직접 호출은 보조 수단일 뿐 — 6cCA가 bulkCreateNodes를 직접 호출해
> 폼/입고 경로의 결함(blade_slot 미저장)을 놓친 전례. 신규 기능은 **동일 데이터를 만드는 인접
> 경로(입고 등)까지 커버 범위에 포함**한다.
