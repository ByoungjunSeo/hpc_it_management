...
> 작성일: 2026-06-26 / 최종 현행화: 2026-07-31 (BUG-19 마이그 사진 참조 파괴 정정)
> 작성 목적: 서버 종료 전 맥락 보존. 재개 시 이 문서 + git log부터 확인.

> ★ **BUG-18 오진→BUG-19 근본 규명·정정 (2026-07-31)** — 자산 상세 엉뚱한 사진.
> - **BUG-18은 오진**: "관리번호 조인 누수"로 봤으나, 격리 PASS는 실재 않는 시나리오 시딩. 실제 원인 아님.
> - **BUG-19(근본)**: 7/11 B-7f 컷오버가 EUL을 재번호(1 usage→N 이벤트)했는데 `migrate-data.js`가 photos.entity_id를
>   remap 안 함 + FK 없음 + verify가 eul-사진 미검사 → 입출고 사진이 엉뚱한 로그/자산을 가리킴.
> - **정정 완료(데이터)**: v1(app/data/it_assets.db)로 원주인 확정 → 오귀속 10장 재연결(유니와이드-004/005·글루시스-006).
>   백업 `backup/photos_pre_bug18fix_20260731.sql`(평문 → 폐기목록). 트랜잭션 4/4/2 검증 COMMIT.
> - **관례**: 이력성 참조 귀속은 재사용 관리번호가 아니라 **id 보존 식별자(asset_id)로**. 마이그 시 **재번호 테이블의
>   참조처(photos.entity_id·audit_logs.target_id) 반드시 remap+검증**. 상세 BUG_TRACKING BUG-18~23.
> - **잔여 과제**: BUG-20(photos FK) · BUG-21(audit target_id stale ~205, 재작성 안 함) · BUG-22(입고 asset_id 기입) ·
>   BUG-23(EUL 감사 details) · BUG-18 코드(asset_id 조인)는 BUG-22 이후 재설계.

>
> ~~★ **BUG-18 자산 상세 입출고 사진 누수 (2026-07-31)**~~ —
> **[오진·폐기] 아래 원인·수정 기술은 전부 틀림. 실제 원인은 상단 BUG-19 블록 참조.**
> *(오진 경위 보존용으로만 남김. 판단 근거로 사용 금지.)*
> - (폐기) **증상**: 방금 입고한 글루시스-009 상세에 과거(4/28) 입출고 사진이 딸려 나옴.
> - (폐기) **원인**: `photo.js findByAssetWithUsageLogs`가 입출고 사진을 재사용 가능한 `management_number`로 조인 →
>   append-only 이력 + 재입고(asset_id 유지)/번호 재사용 시 과거·타 자산 사진 누수.
> - (폐기) **수정**: 조인을 **`asset_id` FK 기준**으로. 시그니처 유지(managementNumber 미사용). 격리 검증 PASS(실제 모델 경로).
> - (폐기) **관례**: 이력성 사진/데이터 귀속은 **재사용 가능한 관리번호가 아니라 asset_id(안정 식별자)로**. 상세 BUG_TRACKING BUG-18.
> - (폐기) **후속(별건, 결정됨)**: 업체 관리번호 형식 `업체명-자산유형(한글)-번호`(예: 글루시스-스위치-001) + 기존 일괄
>   마이그레이션. 코드는 `generateVendorManagementNumber`(vendor,type)로 확장, 마이그레이션은 게이트(운영 DB 쓰기).

> ★ **v2.2.1 릴리스 준비 (2026-07-22, 문서·버전 표기 — 코드 로직 무변경)** — v2.2.0 태그 이후 수리분 재패키징.
> - **포함 수리**: BUG-14(사용현황 실배선) · BUG-15/15-b/15-c(반납 재고 연쇄 차감+모듈 removed 이력, 소급 보정 게이트) ·
>   BUG-16(사용현황 GROUP BY·이동 병합) · BUG-17(Ollama 'AI 스펙 조회' 제거·외부 IP 노출 소멸).
> - **버전 표기 갱신**: `v2/app/package.json` 2.2.0→2.2.1, `docker-compose.prod.yml` 기본 태그·tar 주석 2.2.1,
>   `DEPLOY.md` 빌드/기대태그 2.2.1. **package-lock(multer/pg-types 2.2.0)·과거 릴리스노트·V2.1_BACKLOG의 2.2.0은
>   존치**(서드파티 dep·과거 사실).
> - **★ DDL 없음 확정**: v2.2.0 태그 이후 db/스키마/migrations 변경 **0건**(`git diff --stat v2.2.0..HEAD -- *.sql db`).
>   → 업그레이드는 **이미지 교체 + 재시작만**(마이그레이션 실행 불필요). 리줌노트 `RELEASE_NOTICE_2.2.1.md`.
> - **사무실 빌드 대기**: `it-assets-2.2.1.tar` 빌드 후 sha256을 릴리스노트 `<빌드 후 기입>`에 채움 + v2.2.1 태그(사용자 수동).
> - 상세: `RELEASE_NOTICE_2.2.1.md`, 리포트 `/tmp/6cCV_report.md`.

> ★ **반납 컴퓨팅 모듈 이력 BUG-15-b/c (2026-07-22)** — BUG-15 후속.
> - **BUG-15-c(자동 기록, 코드)**: 반납 트랜잭션 안에서 각 반납 자산(섀시 연쇄·노드 단독)의 컴퓨팅 모듈마다
>   `module_inventory_logs`에 **removed 이력** 추가(quantity_change=count, notes='반납 (자동 기록) …'). **재고는
>   BUG-15 recalc가 처리 → 이력만 추가(이중 차감 없음).** 모듈 0개 자산은 이력 없이 통과.
> - **BUG-15-b(소급 보정, 게이트 SQL)**: 구 코드로 반납된 글루시스-008 N1/N2 18모듈 removed 이력 소급 INSERT
>   (`/tmp/glusys008_history_fix.sql`, notes='반납 소급 보정' — 자동분과 구분, idempotent, 재고 무영향).
> - **★ 관례**: 반납은 **재고 recalc + 모듈 이력 removed 동시 기록**(자동). 자동/수동 소급은 notes로 식별.
> - 상세: BUG_TRACKING.md BUG-15/15-b/15-c.

> ★ **컴퓨팅 모듈 트래킹 BUG-15/16 + Ollama 제거 BUG-17 (2026-07-22)** — 6cCQ 조사 후 수리.
> - **BUG-15**(반납 시 모듈 재고 미차감·노드 사용중 잔존): 반납 라우트가 **섀시 반납 시 자식 노드까지 한 트랜잭션
>   연쇄 반납**(status·EUL) + 커밋 후 **recalculateInUse()** 호출. 노드 단독 반납도 동일. (markReturned·
>   returnActiveByManagement에 client 인자 추가.)
> - **BUG-16**(이동 시 중복 행): 이동을 **동일 슬롯(item_code+slot_info+owner) 수량 병합 UPSERT**로(다른 slot은
>   보수적 비병합) + getUsageByCode **GROUP BY 표시 이중 방어**. 기존 중복 2그룹 보정 SQL(`/tmp/bug16_data_fix.sql`,
>   게이트 — 코드 반영·재시작 후 실행).
> - **BUG-17**(AI 스펙 조회 Ollama 제거, 커밋됨): specLookup.js 삭제·라우트/버튼/JS·OLLAMA_* env·DEPLOY 정리.
>   (별개 미구현 'AI 채팅'/chat 주석은 존치.)
> - **★ 관례 보강**: 반납은 **자식 노드·컴퓨팅 모듈 재고(recalc)까지 연쇄** / 부품 이동은 **동일 슬롯 수량 병합**
>   (다른 슬롯 보수적 유지) / 재고 표시는 GROUP BY 이중 방어.
> - 상세: BUG_TRACKING.md BUG-15/16/17. 예약 ~7/27 BL-11 평문 덤프 폐기 유지.

> ★ **v2.2.0 확정 (2026-07-16)** — 원격 접근 스위트(T1 전원·T2 SSH·T3 SOL·T4 백업) + BL-2 블레이드 일괄 등록 +
> BL-13 대여 장부 + BUG-9/10/12/13 해소.
> - **태그**: `v2.2.0` = 커밋 **62c2603**(docs bug12-13, BUG-12/13 반영). **앱 이미지 tar `it-assets-2.2.0.tar`
>   sha256 `ED15DB06DE4694CC6BD7B72AB514FB43EE24CB35FE53CF2D0EF0333AB925CF19`**(이미지 `it-assets:2.2.0`, 태그
>   커밋에서 빌드 — deb.debian.org 차단이라 kakao 미러 ARG 경유). dist 번들 `it-assets-dist-2.2.0.tar.gz`.
> - **신규 설치 검증 통과(Windows compose)**: 자산 상세·랙 팝업·블레이드 노드·백업 관리 **우아한 실패**(compose는
>   CLI 백업)·[SSH/BMC 콘솔/BMC 웹] 버튼 판정 — **볼륨 청결(down -v) 상태**에서 확인. BUG-12(잔류 pgdata 볼륨)·
>   BUG-13(백업 경로 EACCES) 종결.
> - **BUG-14 수리(태그/tar 이후)**: 부품 "사용 현황" 탭 배선(getUsageByCode) — 6cCO 조사, 격리 검증 PASS. **배포
>   tar에는 미포함**(빌드·해시 확정 후 수리). §문서 확정(sha256 기입·`.gitattributes` CRLF 방지·DEPLOY 트러블슈팅)도 동일.
>   → **v2.2.0 태그·tar는 그대로 고정**, BUG-14+문서는 **다음 재패키징(v2.2.1) 합류** 권장(tar-태그 일관성).
> - **Unix 컴퓨팅모듈 조사(6cCN)**: sshDiscovery는 리눅스 전용 스크립트 → 리눅스=스캔 동작(sudo 필요), 비-리눅스
>   유닉스=우아한 빈 결과 → **수동 입력**(computing_modules 스키마 OS 무관). 입고 시 참고.
> - 예약: **~7/27 BL-11 평문 덤프 폐기** 유지.

> ★ **v2.2.0 릴리스 차단 BUG-12/13 (2026-07-16, 수리 완료·운영 반영 대기)** — Windows compose 신규 설치 스모크 발견.
> - **BUG-12**(자산 상세 `password_enc does not exist`): **HEAD 스키마 결함 아님**으로 판명. 신규 설치 스키마(01~06)는
>   password_enc 포함(compose init 정확 재현·마이그레이션 산물 전수 감사·라이브 쿼리로 실증). **근본원인=재설치 시
>   남은 `pgdata` 명명 볼륨 재사용(초기화 미실행)** 또는 pre-BL-11 dist. → 방어: **기동 스키마 자체점검**(누락 컬럼 시
>   명확한 진단 로그 + `down -v`/마이그레이션 안내) + DEPLOY 재설치 볼륨 청결 안내.
> - **BUG-13**(백업 페이지 `EACCES mkdir /backups/db`): 컨테이너에서 BACKUP_DIR 기본값이 루트 절대경로화 →
>   **우아한 실패**(dirStatus·빈 목록·페이지 안내, 대시보드 에러 전파 차단). compose 백업은 CLI(§5-1).
> - **★ 검증 관례 보강(이번 마디 확립)**: ① **스키마 동등성은 "fresh vs 마이그레이션 전수 적용"의 객체(컬럼·인덱스·
>   제약) 전수 diff/감사**로(단일 baseline 비교는 baseline에 이미 접힌 결함을 못 잡음). ② **릴리스 스모크에 상세
>   페이지·[보기]·에러 배너 확인 포함**, 신규 설치는 **volume 청결(down -v)** 상태에서. ③ **compose 경로 필수**로 검증
>   (systemd 경로만 보면 컨테이너 전용 결함을 놓침 — BUG-13이 그 사례).
> - **⚠ 태그 재이동 필요**: v2.2.0 태그는 아직 미이동(6cCL 커밋 위). BUG-12/13 수리 커밋 반영 후 **v2.2.0 태그를
>   수리 커밋으로 재이동**(사용자 수동). dist 재빌드도 이 커밋 기준.

> ★ **BUG-9/10 종결 (2026-07-15, 운영 반영·실장비 인수 완료)**: BL-2 후속 조사 2건 + BUG-9 후속.
> **BUG-9** — 자식 노드 사용등록이 부모 섀시 위치를 덮어씀(inventory.js 동기화 블록 원인) →
> 블록 제거 + 모델 가드(자식은 위치 필드 강제 NULL) + 폼 위치 숨김. **BUG-9 후속** — 사용등록·수정
> 폼이 노드에 임의 위치 입력을 허용해 EUL 이력이 부모와 어긋나던 구멍 → 폼 잠금(unit 프리필 포함) +
> 서버 검증(부모 위치 불일치 거절, 이중 방어). **BUG-10** — 다노드 입고 노드의 blade_slot 미저장 +
> blade_slot 의미 충돌(렌더용 left/right vs 노드슬롯) → **node_index 신설**, blade_slot 렌더용 원복,
> 부분 유니크 인덱스 (parent, node_index)로 교체, 입고·BL-2 양 경로 node_index 부여.
> 마이그레이션 `db/migrations/2026-07-15_1_bug9_10_node_index.sql` 운영 적용 완료(운영 DB에 node_index
> 존재 확인). **관련 커밋 3건**: `fix(bug9-10)` · `docs` · `fix(bug9)`. 상세: BUG_TRACKING.md BUG-9/10.
>
> ★ **정리·검증 실측(2026-07-15, 운영 SELECT)**:
> - **테스트 데이터(test-001 계열) 전부 정리 완료**(운영 잔존 0건). 섀시 삭제 시 자식 자동 삭제는
>   FK `assets.parent_asset_id … ON DELETE CASCADE`로 보장됨(운영 스키마 delete_rule=CASCADE 확인).
> - **EUL 과거분 점검 — 주의(예상과 다름)**: 자식 노드의 최신 in_use 위치가 부모와 어긋나는 건이
>   **2건 존재**(글루시스-007-N1/N2 = id 1181/1182: 최신 in_use는 Rack#3 U15-U16/16-17, 부모
>   글루시스-007은 현재 Rack#4 U33-U34). 이는 BUG-9 후속의 "임의 위치 허위입력"이 아니라 **섀시를
>   Rack#3→Rack#4로 재배치(2026-06-30 in_use)한 뒤 노드를 재등록하지 않아 생긴 과거 드리프트**
>   (노드 이력은 당시 실제 위치라 그 자체로 허위는 아님). 글루시스-008-N1/N2는 부모와 일치.
>   → **보정 여부는 게이트 질문**(과거 이벤트 이력 유지 vs 현행 위치로 재등록). "모순 실데이터 없음"으로
>   단정하지 말 것.
>
> ★ **검증 관례(확립)**: **격리 검증은 HTTP 실경로 필수**(로그인 세션 curl/fetch로 라우트를 실제로
> 탄다). 모델 직접 호출은 보조 수단일 뿐 — 6cCA가 bulkCreateNodes를 직접 호출해 폼/입고 경로의 결함
> (blade_slot 미저장)을 놓친 전례. 신규 기능은 **동일 데이터를 만드는 인접 경로(입고 등)까지 커버**.

> ★ **릴리스 현황(2026-07-14)**: **v2.1.0 종결 — 타 팀 배포 첫 정식본**
> (태그 `v2.1.0`=4e3a0b4, dist `v2/backups/it-assets-dist-2.1.0.tar.gz` sha256 `418f2152…f88a`).
> **신규 설치 + 업그레이드 양 경로 모두 문서만으로 완주 검증**. v2.1.0 = 2.0.2 + BL-11(자격증명
> AES-256-GCM 암호화·[보기]/[복사]) + BL-12(재고 점검 MVP+원복). 2.0.1·2.0.2·v2.1.0 **전부 현재
> 미발송**(2.0.x는 사내 검증본, v2.1.0은 발송 가능 상태로 보관 — 시점 자유). 상세: 아래
> "★ v2.1.0 릴리스 종결" 섹션.
>
> ★ **태그 이후 완결(릴리스 미포함)**: **T1 전원제어 수리(BUG-2 종결, 실장비 인수 통과)** +
> **BL-13 대여 장부 완료**(격리 47건 + 브라우저 인수 통과) — 둘 다 v2.1.0 태그 이후 커밋이라
> 현 dist에는 없다. 발송 시점 도래 시 그때 HEAD 기준 재패키징(v2.1.1 또는 v2.2). 상세: 아래
> "★ T1·BL-13 완결" 섹션.

> ★ **새 세션은 여기부터**: 최신 상태는 —
> "★ v2.1.0 릴리스 종결(2026-07-14)" · "★ T1·BL-13 완결(2026-07-14)" · "★ 다음 마디: T2 웹 SSH
> 터미널" / "★ 대기/예약 항목(2026-07-14 기준)" / (이력) "★ 2.0.1 배포 준비 종결(3관문+D)" ·
> "★ v2.1 우선순위 확정 + 2.0.2 후보 묶음 완료".
> 그 위·아래의 B-3~B-7 기록은 완결 이력이다.
> 운영 구도(v2 단일 운영본 :3001 systemd it-assets-v2, v1 READ_ONLY :3000)는
> "★ B-7 전체 완결" 섹션의 현 상태 선언이 여전히 유효.

## ★ v2.1.0 릴리스 종결 (2026-07-14)

**타 팀 배포 첫 정식본. 신규 설치·업그레이드 양 경로 문서 완주 검증 완료.**

- **전달물**: `v2/backups/it-assets-dist-2.1.0.tar.gz` (약 193MB, 18파일) + `.sha256`.
  sha256 `418f21528a83742a275bd7f41847ace15f4ea072d01dadee6b11a678c2adf88a`. 태그 `v2.1.0`=4e3a0b4.
- **범위**: 2.0.2(검색 관리번호·선반 1/3U·실 중복 방지·SESSION_SECRET 강제) + BL-11(자격증명
  암호화·노출점 차단·[보기]/[복사]) + BL-12(재고 점검 MVP+원복). BL-8 redact는 데이터 보정(패키지 무관).
- **발송 상태**: 2.0.1/2.0.2 **미발송·대체**, v2.1.0도 **현재 미발송** — 발송 가능 상태로 보관
  (서버 `v2/backups/` + 사무실 PC `C:\it-assets-releases\2.1.0\`). 발송 시점 자유.

**이번 릴리스에서 확립된 배포 관례(다음 릴리스도 준용):**
- **db/migrations/ 순번 명명**: `날짜_N_이름`(예: `2026-07-13_2_bl12_inventory_audit.sql`).
  `*.sql` glob 정렬이 **의존성 적용 순서를 보장**(테이블 CREATE → 그 테이블 ALTER 순).
- **encrypt-credentials.js는 `app/scripts/`**(앱 이미지 동봉) — dist 호스트 실행은 pg·
  credentialCrypto 의존으로 불가하므로 **컨테이너 exec**(`docker compose exec -T app node
  scripts/encrypt-credentials.js`)로 실행. backup.sh(호스트 docker exec 방식)와 대조.
- **재검증에 업그레이드 시뮬 필수**: 클린 설치만이 아니라 이전 dist→신 dist 업그레이드를
  §5 문서만으로 완주하는 검증을 넣는다.

**U-1/U-2 발견·수정 경위(업그레이드 시뮬의 가치 실증):** 초판(선택 A 후보)에서 업그레이드
시뮬 중 **U-1**(migrations 알파벳 정렬이 correction_revert를 inventory_audit보다 먼저 실행 →
ALTER 대상 테이블 없음)·**U-2**(encrypt가 v2/scripts라 이미지·dist 미동봉 → 컨테이너 실행 불가)
발견 → 선택 B로 순번 rename + app/scripts 이동으로 근절, 재검증에서 §5 완주 확인(4e3a0b4).
클린 설치만 봤으면 놓쳤을 결함 — 업그레이드 시뮬을 상시 관례로 채택한 근거.

## ★ T1·BL-13 완결 (2026-07-14, v2.1.0 태그 이후 — 현 dist 미포함)

두 건 모두 **v2.1.0 태그(4e3a0b4) 이후 커밋**이라 발송된 dist에는 없다. 다음 발송 시 재패키징 대상.

- **T1 전원제어 수리 — BUG-2 종결 (커밋 5b8066a)**: racks.js `ipmiPowerControl` 복원(주석 해제),
  화이트리스트 on/off/reset, 자격증명 BL-11 `password_enc` 복호화(power-status 동일 경로), v1
  미정의 변수 잔재 제거. power-control 라우트 503 스텁 → 실동작: **requireAdmin(관리자 한정)**,
  no_bmc/no_cred 실행 전 409 차단 + 버튼 비활성 UX, 명령별 audit(값 미기록), BMC 미응답↔인증
  실패 구분 메시지. `IPMI_DRYRUN=1` 검증훅(실장비 무조작). **격리 8케이스 PASS + 실장비 인수
  테스트 통과.** ※ BUG_TRACKING.md의 BUG-2 서술이 이 종결과 어긋나면 그 파일은 별도로 갱신
  필요(이번 가이드 현행화에서는 미접촉).
- **BL-13 대여 장부 완료 (커밋 b150661)**: 메뉴 노출 + due_date/연체 표시 + 재고 자동 차감/복귀
  (부족 시 거부) + 품목 부분 반납(완납 시 자동 `returned`) + 자산 연결·랙 해제 + vendor FK 매칭 +
...
- **게이트 흐름**: 조사 → ★게이트(사용자 승인) → 구현 → 격리 검증 → **수동 커밋** → 운영 반영
  (DDL 적용 → 재시작) → **브라우저 인수**. 운영 데이터 보정/DDL은 반드시 게이트(BL-8·BL-3에서 확립).
- **일회용 격리 검증 스택**: `blNNtest-db`(postgres:16-alpine, **:15433**, --rm·볼륨 미지정) + 임시 앱
  (**:13001**, `timeout` 한정, APP_PORT/PGPORT env 주입). OPS-1(2026-07-12 해소) 이후 기본 bridge
  정상 — 전용 네트워크는 선택. `__TEST__` 접두 데이터 관례, 종료 시 잔재 0 확인.
- **EJS/클라이언트 JS 검증 표준**: 수정 뷰의 인라인 `<script>`를 `new Function(body)`(또는
  실렌더 페이지 GET 후) 파싱해 문법 통과 확인.
- **신규 설치 스키마 자체 완결**: `db/01~06_*.sql`이 전 기능(마이그레이션 결과 포함)을 자체 포함 —
  신규 설치는 migrations 불요. `db/migrations/`는 기존 DB 업그레이드용이며 **순번 명명(날짜_N_이름)**
  으로 적용 순서 보장. **릴리스 재검증에 업그레이드 시뮬(이전 dist→신 dist) 필수**.
- 운영 반영(재시작·DDL 적용 포함)은 **항상 사용자 수동** — 에이전트는 수정·격리 검증·완성형 명령 제시까지.
- **커밋 메시지에 `Co-Authored-By` 트레일러 금지**(2026-07-13 이력 재작성으로 제거 — 재유입 방지).

## ★ 대기/예약 항목 (2026-07-14 기준)
...
> 채널(컴퓨팅지원팀 sbj8388@tta.or.kr 확정)·inventory_count_*.js(BL-12 설계 시 옵션B 공식 승계 후
> 삭제 처분)·/tmp 리포트(각 세션 리포트는 필요 시 개별 이동).
