...
> 작성일: 2026-06-26 / 최종 현행화: 2026-07-31 (BUG-18 자산 사진 누수 수정)
> 작성 목적: 서버 종료 전 맥락 보존. 재개 시 이 문서 + git log부터 확인.

> ★ **BUG-18 자산 상세 입출고 사진 누수 (2026-07-31, 수정 완료·운영 반영 대기)** — 관리번호 재사용 이슈.
> - **증상**: 방금 입고한 글루시스-009 상세에 과거(4/28) 입출고 사진이 딸려 나옴.
> - **원인**: `photo.js findByAssetWithUsageLogs`가 입출고 사진을 재사용 가능한 `management_number`로 조인 →
>   append-only 이력 + 재입고(asset_id 유지)/번호 재사용 시 과거·타 자산 사진 누수.
> - **수정**: 조인을 **`asset_id` FK 기준**으로. 시그니처 유지(managementNumber 미사용). 격리 검증 PASS(실제 모델 경로).
> - **관례**: 이력성 사진/데이터 귀속은 **재사용 가능한 관리번호가 아니라 asset_id(안정 식별자)로**. 상세 BUG_TRACKING BUG-18.
> - **후속(별건, 결정됨)**: 업체 관리번호 형식 `업체명-자산유형(한글)-번호`(예: 글루시스-스위치-001) + 기존 일괄
